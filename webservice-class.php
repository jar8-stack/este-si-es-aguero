...
class WebserviceBoxito {
	...
	{
		$exist =0;
			$con = mysqli_connect('3.141.3.224', 'test1', 'Box%%1234.','PARCIAL3');
		if (!$con) 
		{
	...
	function saveimg($base_img,$id_report)
	{
	...
	$file = UPLOAD_DIR . uniqid() . '.png';
				...
				$sql = "INSERT INTO Imagenes (nombreImagen,url,idReport) values ('img_".$id_report."','','".$id_report."')";
    ...
    function add_report($json) 
	{
	    ...
	    foreach($json->reporte->evidencias as $ev)
	    {
	        saveimg($ev,$json->reporte->idUsuario );
	    }
		...
		if(isset($json->reporte))
			{
				...
				$con = mysqli_connect('3.12.73.95', 'said_remoto', 'clave1','usuarios');
				if (!$con) 
				{
				  die("Connection failed: " . mysqli_connect_error());
				}
  ...
  function update_user($json) 
	...
	function get_user($json)
	{
		...
		if(isset($json->usuario))
		{
			...
			$consulta="SELECT * from usuario where idUsuario =  ".$json->usuario->idUsuario."";
			$resultado = mysqli_query( $con, $consulta ) or die ( "Error: 001"); 
			$row_cnt = $resultado->num_rows;
			$c = 0;
			while ($columna = mysqli_fetch_array( $resultado ))
